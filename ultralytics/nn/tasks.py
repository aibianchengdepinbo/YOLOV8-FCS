...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m,
                      (Detect, Detect_DyHead, DetectDBB, DetectACDBB, DetectAC, DetectDeepDBB, DetectDeepACDBB,
                       Detect_Efficient, Detect_DyHead_SDBB, Detect_DyHead_ALLDBB, Detect_Efficient3DBB, Detect_Efficient3DBBR, Detect2ACDBB, Detect2AC2DBB, Detect2DBBAC, Detect2DBB2AC,
                       DetectSingleDBB, DetectAux, DetectAuxDBB, Segment)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m,
                      (Detect, Detect_DyHead, DetectDBB, DetectACDBB, DetectAC, DetectDeepDBB, DetectDeepACDBB,
                       Detect_Efficient, Detect_DyHead_SDBB, Detect_DyHead_ALLDBB, Detect_Efficient3DBB, Detect_Efficient3DBBR, Detect2ACDBB, Detect2AC2DBB, Detect2DBBAC, Detect2DBB2AC,
                       DetectSingleDBB, DetectAux, DetectAuxDBB, Segment, Pose)):
            s = 640  # 2x min stride
            m.inplace = self.inplace
            if isinstance(m, (DetectAux, DetectAuxDBB)):
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (
        nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead, DetectDBB, DetectACDBB, DetectAC, DetectDeepDBB, DetectDeepACDBB,
        Detect_Efficient, Detect_DyHead_SDBB, Detect_DyHead_ALLDBB, Detect_Efficient3DBB, Detect_Efficient3DBBR, Detect2ACDBB, Detect2AC2DBB, Detect2DBBAC, Detect2DBB2AC,
        DetectSingleDBB, DetectAux, DetectAuxDBB, Segment):
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        ...
        DetectAux, DetectAuxDBB, Detect_Efficient, Detect_DyHead_SDBB, Detect_DyHead_ALLDBB, Detect_Efficient3DBB, Detect_Efficient3DBBR, Detect2ACDBB, Detect2AC2DBB, Detect2DBBAC, Detect2DBB2AC,
        DetectSingleDBB, Segment):
            m.inplace = inplace  # torch 1.7.0 compatibility
        elif t is nn.Upsample and not hasattr(m, 'recompute_scale_factor'):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus, Bottleneck_ECA,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, C2f_Faster, C2f_ODConv,
                 C2f_Faster_EMA, C2f_ACDBB, C2f_DeepACDBB, C2f_DeepDBB, C2f_DeepACDBBMix, C2f_DBB, ACBlock, ACBlockDBB, C2f_ACNET, GSConv, VoVGSCSP, VoVGSCSPC, C2f_CloAtt, C3_CloAtt, SCConv,
                 C2f_SCConv, C3_SCConv, C2f_ScConv, C3_ScConv,
                 C3_EMSC, C3_EMSCP, C2f_EMSC, C2f_EMSCP, RCSOSA, C2f_KW, C3_KW, C2f_DySnakeConv, C3_DySnakeConv, Bottleneck_CAttention,
                 DCNv2, C3_DCNv2, C2f_DCNv2, DCNV3_YOLO, C3_DCNv3, C2f_DCNv3, C3_Faster, C3_Faster_EMA, C3_ODConv, Bottleneck_SEAttention, Bottleneck_MLCA,
                 OREPA, OREPA_LargeConv, RepVGGBlock_OREPA, C3_OREPA, C2f_OREPA, C3_DBB, C3_REPVGGOREPA, C2f_REPVGGOREPA, Bottleneck_EMA, Bottleneck_CBAM,
                 C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic, C3_ContextGuided, C2f_ContextGuided, C3_MSBlock, C2f_MSBlock, Bottleneck_EMA, Bottleneck_LSKA,
                 C3_DLKA, C2f_DLKA, CSPStage, SPDConv, RepBlock, C3_EMBC, C2f_EMBC, SPPF_LSKA, C3_DAttention, C2f_DAttention, C3SECA):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Faster, C2f_ODConv, C2f_Faster_EMA, C2f_DBB, C2f_ACNET,
                     VoVGSCSP, VoVGSCSPC, C2f_CloAtt, C3_CloAtt, C2f_SCConv, C3_SCConv, C2f_ScConv, C3_ScConv,
                     C3_EMSC, C3_EMSCP, C2f_EMSC, C2f_EMSCP, C2f_ACDBB, RCSOSA, C2f_KW, C3_KW, C2f_DySnakeConv, C3_DySnakeConv,
                     ...
                     C3_MSBlock, C2f_MSBlock, C3_DLKA, C2f_DLKA, CSPStage, RepBlock, C3_EMBC, C2f_EMBC, C3_DAttention, C2f_DAttention, C3SECA):
        ...
        elif m in (Detect, Detect_DyHead, DetectDBB, DetectACDBB, DetectAC, DetectDeepDBB, DetectDeepACDBB,
                   Detect_Efficient, Detect_DyHead_SDBB, Detect_DyHead_ALLDBB, Detect_Efficient3DBB, Detect_Efficient3DBBR, Detect2ACDBB, Detect2AC2DBB, Detect2DBBAC, Detect2DBB2AC, DetectSingleDBB,
                   DetectAux, DetectAuxDBB, Segment, Pose, RTDETRDecoder):
            ...
            c2 = ch[f] * 2
            args = [ch[f], c2, *args]
        elif m in {BiFusion, DLAFusion, DLAFusionSim}:
            c1 = [ch[x] for x in f]
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect, Detect_DyHead, DetectDBB, DetectACDBB, DetectAC, DetectDeepDBB, DetectDeepACDBB
                               ,Detect_Efficient, Detect_DyHead_SDBB, Detect_DyHead_ALLDBB, Detect_Efficient3DBBR, Detect_Efficient3DBB, Detect2ACDBB,
                              Detect2AC2DBB, Detect2DBBAC, Detect2DBB2AC, DetectSingleDBB, DetectAux, DetectAuxDBB,
                              )):
                return 'detect'
