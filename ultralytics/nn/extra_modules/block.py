...
from ..modules.conv import Conv, DWConv, RepConv, autopad, ACBlock
from ..modules.block import *
from .attention import *
from .rep_block import DiverseBranchBlock, ACBlockDBB, DeepACBlockDBB, DeepDiverseBranchBlock, SegNext_AttentionAC
...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv',
           'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB', 'Bottleneck_SEAttention',
           'GSConv', 'VoVGSCSP', 'VoVGSCSPC', 'C2f_CloAtt', 'C3_CloAtt', 'SCConv', 'C3_SCConv', 'C2f_SCConv', 'ScConv',
           'C3_ScConv', 'C2f_ScConv',
           'LAWDS', 'DyHeadBlockDBB', 'EMSConv', 'EMSConvP', 'C3_EMSC', 'C3_EMSCP', 'C2f_EMSC', 'C2f_EMSCP', 'RCSOSA',
           'C3_KW', 'C2f_KW', 'C2f_DeepACDBB', 'C2f_DeepACDBBMix', 'C2f_DeepDBB',
           'C3_DySnakeConv', 'C2f_DySnakeConv', 'DCNv2', 'C3_DCNv2', 'C2f_DCNv2', 'DCNV3_YOLO', 'C3_DCNv3', 'C2f_DCNv3',
           'FocalModulation', 'Bottleneck_LSKA', 'Bottleneck_CBAM',
           'C3_OREPA', 'C2f_ACNET', 'C2f_ACDBB', 'C2f_OREPA', 'C3_DBB', 'C3_REPVGGOREPA', 'C2f_REPVGGOREPA',
           'C3_DCNv2_Dynamic', 'C2f_DCNv2_Dynamic','Bottleneck_ECA',
           'SimFusion_3in', 'SimFusion_4in', 'IFM', 'InjectionMultiSum_Auto_pool', 'PyramidPoolAgg', 'AdvPoolFusion',
           'TopBasicLayer', 'Bottleneck_EMA', 'Bottleneck_MLCA',
           'C3_ContextGuided', 'C2f_ContextGuided', 'C3_MSBlock', 'C2f_MSBlock', 'ContextGuidedBlock_Down', 'C3_DLKA',
           'C2f_DLKA', 'CSPStage', 'SPDConv', 'Bottleneck_CAttention',
           'BiFusion', 'DLAFusion', 'DLAFusionSim', 'RepBlock', 'C3_EMBC', 'C2f_EMBC', 'SPPF_LSKA', 'C3_DAttention',
           'C2f_DAttention']
...
class Bottleneck_ECA(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3),
        ...
        self.eca = EfficientChannelAttention(c2)
...
class Bottleneck_CAttention(nn.Module):
    """Standard bottleneck."""
...
class Bottleneck_LSKA(nn.Module):
    ...
    def forward(self, x):
        """'forward()' applies the YOLOv5 FPN to input data."""
        return x + self.lska(self.cv2(self.cv1(x))) if self.add else self.lska(self.cv2(self.cv1(x)))
...
class Bottleneck_SEAttention(nn.Module):
    """Standard bottleneck."""
...
class DyHeadBlockDBB(nn.Module):
    """DyHead Block with three types of attention.
    HSigmoid arguments in default act_cfg follow official code, not paper.
    https://github.com/microsoft/DynamicHead/blob/master/dyhead/dyrelu.py
    """
    ...
    def __init__(self,
        ...
        # self.spatial_conv_offset = nn.Conv2d(
        #     in_channels, self.offset_and_mask_dim, 3, padding=1)
        self.spatial_conv_offset = DiverseBranchBlock(
            in_channels, self.offset_and_mask_dim, kernel_size=3, padding=1)
        self.scale_attn_module = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, 1, 1),
            nn.ReLU(inplace=True), build_activation_layer(act_cfg))
        self.task_attn_module = DyReLU(in_channels)
...
class Bottleneck_SegACAtt(Bottleneck):
    """Standard bottleneck With CloAttention."""

    def __init__(self, c1, c2, shortcut=True, g=1, k=..., e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        self.attention = SegNext_AttentionAC(c2)
...
class C2f_SegACAtt(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_SegACAtt(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class DLAFusion(nn.Module):
    '''DLAFusion Block in PAN'''

    def __init__(self, in_channels, out_channels):
        super().__init__()
        # print(in_channels,out_channels)
        self.cv1 = Conv(in_channels[1], out_channels, 1, 1)
        self.cv2 = Conv(out_channels * 2, out_channels, 1, 1)
    ...
    def forward(self, x):
        ...
        return self.cv2(torch.cat((x0, x1), dim=1))


class DLAFusionSim(nn.Module):
    '''DLAFusion Block in PAN'''
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # print(in_channels,out_channels)
        # self.cv1 = Conv(in_channels[1], out_channels, 1, 1)
        self.cv2 = Conv(sum(in_channels), out_channels, 1, 1)
    ...
    def forward(self, x):
        ...
        return self.cv2(torch.cat((x0, x[1]), dim=1))
...
class FeatureConcatenation(nn.Module):
    def __init__(self, c1, c2, kernel_size=3):
        super(FeatureConcatenation, self).__init()
        self.conv_1 = Conv(c1, c2, k=3, s=1, p=1, g=1, d=1, act=True)
        self.conv_2 = Conv(c2, c2, k=3, s=1, p=1, g=1, d=1, act=True)

    def forward(self, shallow_features, deep_features):
        # 上采样浅层特征
        upsampled_shallow_features = F.interpolate(shallow_features, scale_factor=2, mode='nearest')
        upsampled_shallow_features = self.conv_1(upsampled_shallow_features)

        # 将上采样后的浅层特征和深层特征进行拼接
        concatenated_features = torch.cat((upsampled_shallow_features, deep_features), dim=1)

        concatenated_features = self.conv_2(concatenated_features)
        return concatenated_features


class FeatureConcatenationAdd(nn.Module):
    def __init__(self, c1, c2, kernel_size=3):
        super(FeatureConcatenation, self).__init()
        self.conv_1 = Conv(c1, c2, k=3, s=1, p=1, g=1, d=1, act=True)
    ...
    def forward(self, shallow_features, deep_features):
        ...
        # 将上采样后的浅层特征和深层特征进行拼接
        concatenated_features = ((upsampled_shallow_features + deep_features))

        return concatenated_features


if __name__ == "__main__":
    a = torch.tensor(1, 64, 80, 80)
    b = torch.tensor(1, 128, 40, 40)
    net = DLAFusion([64, 128], 128)
    c = net(b, a)
    print(c.shape)
